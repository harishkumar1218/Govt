...
import { ToastProvider } from '../components/threads/ToastContext';
import threadsStyles from '../components/threads/Threads.module.css';
...
export default function LandingPage({ 
  ...
  // Threads states
  const [activeTab, setActiveTab] = useState<'learn' | 'threads'>('learn');
  ...
  const handleTabChange = (tab: 'learn' | 'threads') => {
    setActiveTab(tab);
    const params = new URLSearchParams(window.location.search);
    if (tab === 'threads') {
  ...
  return (
    <div className={styles.wrapper}>
      {/* Toast Alert is managed dynamically inside ToastProvider */}

      <header className={`glass-header ${styles.header}`}>
        <div className={`container ${styles.headerInner}`}>
          <div className={styles.logo} style={{ cursor: 'pointer' }} onClick={() => handleTabChange('learn')}>UPSC Aspire</div>
      ...
      <main className="container">
        {/* Learn vs Threads Tab Toggle */}
        <div className={threadsStyles.tabToggleContainer}>
          <div className={threadsStyles.tabToggle}>
            <div 
              className={threadsStyles.tabIndicator} 
              style={{ transform: activeTab === 'learn' ? 'translateX(0)' : 'translateX(100%)' }} 
            />
            <button
              className={`${threadsStyles.tabBtn} ${activeTab === 'learn' ? threadsStyles.activeTabBtn : ''}`}
              onClick={() => handleTabChange('learn')}
            >
              Learn
            </button>
            <button
              className={`${threadsStyles.tabBtn} ${activeTab === 'threads' ? threadsStyles.activeTabBtn : ''}`}
        ...
        {activeTab === 'learn' ? (
          ...
          /* Threads tab content */
          <ToastProvider>
            <ThreadsShell userId={user ? user.id : 0} />
          </ToastProvider>
        )}
      </main>
    </div>
